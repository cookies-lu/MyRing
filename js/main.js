...
var dteIn = document.getElementById("dteIn")
var saveVisible = document.getElementById("saveBtn");
var daysCircle = document.getElementById("days");
var logoCircle = document.getElementById("logo");
var daysIn = document.getElementById("daysIn")
var slogan = document.getElementById("slogan")
var cycleOut = document.getElementById("cycleOut");
var daysOut = document.getElementById("daysOut")
var dateIn = document.getElementById("dateIn");
var element = document.getElementById("inCalendar");
var element2 = document.getElementById("outCalendar");
var changebtn = document.getElementById("changeCycle");
var cycleIn = document.getElementById("cycleIn");
var daysTakeOff;
var daysToInsert;
var insertCalendar;
var removeCalendar;
...
filechoose.onchange = function(e) {
      ...
      try {
        ...
        arweave.wallets.jwkToAddress(wallet).then((address) => {
          loginHandler(wallet, address);
			...
			saveVisible.classList.add("hidden");
			saveVisible.classList.add("displaynone");
			btnLogout.classList.remove("displaynone");
			btnLogout.classList.remove("hidden");
			btn.classList.add("hidden");
			btn.classList.add("displaynone");
			logoCircle.classList.add("hidden");
			logoCircle.classList.add("displaynone");
			daysCircle.classList.remove("hidden");
			daysCircle.classList.remove("displaynone");
			slogan.classList.add("displaynone");
			slogan.classList.add("hidden");
			changebtn.classList.remove("hidden");
			changebtn.classList.remove("displaynone");

			if ((daysIn =null && daysIn =="") && (daysOut =null && daysOut =="") && (dateIn =null && dateIn =="")) {
				daysIn.classList.remove("hidden");
				daysIn.classList.remove("displaynone");
				daysOut.classList.remove("hidden");
				daysOut.classList.remove("displaynone");
				dateIn.classList.remove("hidden");
				dateIn.classList.remove("displaynone");
  ...
  async function getRing() {
    ...
    savedRing = jsonObj;
    console.log(savedRing);
	console.log(JSON.stringify(savedRing));

	dateOut.classList.remove("hidden");
	dateOut.classList.remove("displaynone");
	...
	function addDays(firstDate,days) {
		return new Date(firstDate.getTime() + days*24*60*60*1000);
	}
	
		element.classList.remove("displaynone");
		element.classList.remove("hidden");
		
		var firstDate = new Date(savedRing[0].dtIn);
		console.log(firstDate);
		
		var newDate = addDays(firstDate,savedRing[0].dIn);
		var dateRemove = formatDate(newDate);

		cycleIn.classList.remove("displaynone");
		cycleIn.classList.remove("hidden");
		cycleIn.innerHTML="You should use the ring during these days:"
		dateOut.innerHTML = "</br>You should remove the ring at: " + dateRemove;
		
		var Date1 = new Date ();
		var Date2 = new Date(dateRemove);
		daysTakeOff = (Math.floor((Date2.getTime() - Date1.getTime())/(1000*60*60*24)) + 1);
		daysCircle.innerHTML = daysTakeOff + " Days left"

		showInCalendar = true;

		 insertCalendar = flatpickr(element, {
			mode: "range",
			dateFormat: "Y-m-d",
			defaultDate: [firstDate, dateRemove],
			inline:true,
		});

		console.log(Date1);
		console.log(dateRemove);
		console.log(daysTakeOff);

	if (daysTakeOff == 0) {

		insertCalendar.destroy();

		function addDays2(secondDate,days) {
			return new Date(secondDate.getTime() + days*24*60*60*1000);
		}
		dateOut.classList.add("hidden");
		dateOut.classList.add("displaynone");

		var secondDate = new Date(dateRemove);
		console.log(secondDate);
			
		var newDate2 = addDays2(secondDate,savedRing[0].dOut);
		var dateToInsert = formatDate(newDate2);

		var Date3 = new Date ();
		console.log(Date3);
		var Date4 = new Date(dateToInsert);
		console.log(Date4);

		daysToInsert = (Math.floor((Date4.getTime() - Date3.getTime())/(1000*60*60*24)) + 1 );
		daysCircle.innerHTML = daysToInsert + " Days left"
		console.log(daysToInsert);

		cycleIn.classList.add("displaynone");
		cycleIn.classList.add("hidden");
		cycleOut.classList.remove("hidden");
		cycleOut.classList.remove("displaynone");
		cycleOut.innerHTML="You should not use the ring during these days:"
		dteIn.innerHTML ="</br> You should start your ring at: " + dateToInsert;

		 removeCalendar = flatpickr(element2, {
			mode: "range",
			dateFormat: "Y-m-d",
			defaultDate: [secondDate, dateToInsert],
			inline:true,
			display:false
				});

		alert("You should remove your ring");

		inCalendar.classList.add("hidden");
		inCalendar.classList.add("displaynone");
		dteIn.classList.remove("hidden");
		dteIn.classList.remove("displaynone");
		
	};

			if(daysToInsert == 0){ 

				alert("You should start your ring");

				removeCalendar.destroy();

				slogan.innerHTML = " Please insert your new cicle details"
				daysIn = document.getElementById("daysIn");
				daysIn.classList.remove("hidden");
				daysIn.classList.remove("displaynone");
				daysOut.classList.remove("hidden");
				daysOut.classList.remove("displaynone");
  ...
  async function sRing() {
    ...
    if (state.loggedIn) {
		...
		console.log(dIn.value)
		console.log(dOut.value)
		console.log(dtIn.value)
		
			daysIn.classList.add("hidden");
			daysIn.classList.add("displaynone");
			daysOut.classList.add("hidden");
			daysOut.classList.add("displaynone");
			dateIn.classList.add("hidden");
			dateIn.classList.add("displaynone");

			rings.push([dIn.value, dOut.value, dtIn.value]);
			console.log(rings);

	  var saveRing = convertArrayToCSV(rings,{ header, seperator:','});	
	      console.log(saveRing)
      ...
      transaction.addTag("Application-ID", "MyRing");
      transaction.addTag("Title", title);
	  await arweave.transactions.sign(transaction, state.jwk);
	  const response = await arweave.transactions.post(transaction);
	  console.log(response);
	  alert("Info saved to the Arweave, please allow some time for it to be mined and refresh this page");
    } else {
      alert("You must be logged in to use MyRing.");
    }
  };
   ...
   async function changeCycle() {
	...
	cycleOut.classList.add("displaynone");
	cycleOut.classList.add("hidden");
	daysIn.classList.remove("hidden");
	daysIn.classList.remove("displaynone");
	daysOut.classList.remove("hidden");
	daysOut.classList.remove("displaynone");
    ...
    console.log("xupa")
	dateOut.classList.add("hidden");
	dteIn.classList.add("displaynone");
	dteIn.classList.add("hidden");
	cycleIn.classList.add("displaynone");
	cycleIn.classList.add("hidden");
	insertCalendar.destroy();
	removeCalendar.destroy();
